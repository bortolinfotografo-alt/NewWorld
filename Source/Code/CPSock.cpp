...
BOOL CPSock::Receive()
{
	...
	if (tReceiveSize == Rest) {
		...
		char temp[256];
		snprintf(temp, sizeof(temp), "Recive Buffer Size > Recive Size, %d - %d", tReceiveSize, Rest);
		Log(temp, "-system", 0);
		printf("%s\n", temp);
		return TRUE;
	}
	if (tReceiveSize == SOCKET_ERROR) {
		char temp[256];
		snprintf(temp, sizeof(temp), "Recive Buffer Error, %d - %d", tReceiveSize, Rest);
		printf("%s\n", temp);
		Log(temp, "-system", 0);
		return FALSE;
	}
...
BOOL CPSock::AddMessage(char *pMsg, int Size)
{
	...
	{
		sprintf(temp, "err,add send_buffer full %d %d %d %d", nSendPosition, Size, pSMsg->Type, Sock);
		Log(temp, "-system", 0);
		return FALSE;
	}
	...
	if (Sock <= 0)
	{
		sprintf(temp, "err,sock send_buffer invalid %d %d %d %d", nSendPosition, Size, pSMsg->Type, Sock);
		Log(temp, "-system", 0);
		return FALSE;
	}
	if (!pMsg) {
		Log(temp, "-system !pMsg", 0);
		return FALSE;
	}	
